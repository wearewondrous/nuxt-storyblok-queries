...
class StoryblokQueries {
  ...
  getStory(path, options = {}) {
    ...
    return this._loadData({ path, options })
  }
  ...
  getDatasource(path, options = {}) {
    const datatype = "datasource_entries"
    const datasource = path
    return this._loadData({ datatype, datasource, options })
  }
  ...
  _loadData({ datatype = "stories", path = "", startsWith, startpage = true, datasource = "", options = {} }) {
    ...
    return this.client
      ...
      .then(res => {
        return {
          ...res.data,
          ...res.perPage && { perPage: res.perPage },
          ...res.total && { total: res.total }
        }
      })
      .catch(res => {
        if (!res.response) {
...
export default (ctx, inject) => {
  const storyblokContentDeliveryAPI = new StoryblokQueries(ctx)

  ctx.$storyblok = storyblokContentDeliveryAPI
  inject("storyblok", storyblokContentDeliveryAPI)
}