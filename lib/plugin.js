...
class StoryblokQueries {
  ...
  getStoryCollection(path, options = {}) {
    const lang = this.getCurrentLang()
    const startsWith = this._getPath(`${lang}/${path}`, options)
    const startpage = options.startpage || false

    return this._loadData({ startsWith, startpage })
  }
  ...
  _loadData({ datatype = "stories", path = "", startsWith, startpage = true, datasource = "" }) {
    const options = {
